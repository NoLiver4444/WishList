...
const getServerError = (err) => {
	if (err.status === 409) {
		if (err.message?.toLowerCase().includes('email')) return 'Этот email уже занят';
		if (err.message?.toLowerCase().includes('login')) return 'Этот логин уже занят';
...
export const RegisterForm = ({onSuccess}) => {
	...
	const handleBlur = (field) => {
		...
		const fieldErrors = {
			login: form.login.length > 0 && form.login.length < 3
				? 'Минимум 3 символа' : null,
			email: form.email.length > 0 && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)
				? 'Некорректный email' : null,
